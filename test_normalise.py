...
// --- FIX 1: stale photo entry re-scores from cached ingredients, no network ---
{
  ...
  assert(rescored.responseData.scoreLabel === 'Food products only', 'unsupported scoreLabel');
...
// --- FIX 2: stale beats nothing when re-scan throws ---
{
  ...
  const mismatched = {
    ...
    sugar100g: '56g',
    sodium100g: '180mg',
    explanation: "We've packed this product with 25.2g of sugar per serving.",
    scanLogicVersion: '10',
    cachedAt: 1,
  };
  const mismatchFb = staleCacheFallbackPayload(mismatched);
  assert(mismatchFb.productName === 'Cadbury Dairy Milk', 'mismatch fallback keeps product');
  assert(mismatchFb.score === 35, 'mismatch fallback keeps score');
  assert(mismatchFb.sugar === '25.2g', 'mismatch fallback keeps nutrition');
  assert(mismatchFb.protein === '4g', 'mismatch fallback keeps protein');
  assert(mismatchFb.sodium === '80mg', 'mismatch fallback keeps sodium');
  assert(mismatchFb.explanation === undefined,
    'version-mismatch fallback must omit the old explanation');
  assert(mismatchFb.explanationPending === true,
    'mismatch fallback must fall through to missing-explanation behaviour');
  assert(!/we've packed/i.test(JSON.stringify(mismatchFb)),
    'old we-voice must not appear anywhere on the mismatch payload');

  const missingVersion = {
    productType: 'food',
    productName: 'Pre-version cache',
    score: 50,
    sugar: '10g',
    explanation: "We've packed this product with sugar.",
    cachedAt: 1,
  };
  const missingFb = staleCacheFallbackPayload(missingVersion);
  assert(missingFb.productName === 'Pre-version cache');
  assert(missingFb.score === 50);
  assert(missingFb.explanation === undefined,
    'docs without scanLogicVersion must omit explanation');
...
assert(SCAN_LOGIC_VERSION === '22', 'SCAN_LOGIC_VERSION must be 22, got ' + SCAN_LOGIC_VERSION);
...
(async () => {
assert(g.SCAN_LOGIC_VERSION === '22', 'exported SCAN_LOGIC_VERSION must be 22');
...
// 1. Normal food with energy + protein + sodium → numeric score
{
  ...
  assert(result.scanLogicVersion === '22', 'normal food stamps logic version 22');
  assert(result.calories100g === '80', 'yogurt calories100g, got ' + result.calories100g);
  ...
  assert(result.caloriesTier === undefined && result.saturatedFatTier === undefined && result.fiberTier === undefined,
    'scored food must not invent calorie/sat-fat/fiber tiers');
...
// 2. Food with only energy is scorable at the gate but nutrition subscore is unavailable
{
...
// 3. Dawn Ultra nutriment shape → no score, fixed explanation, no Haiku
{
  ...
  assert(result.scanLogicVersion === '22', 'Dawn stamps logic version 22');
  ...
  assert(result.calories === null, 'Dawn calories must be null');
  assert(result.calories100g === null, 'Dawn calories100g must be null');
  assert(result.saturatedFat === null, 'Dawn saturatedFat must be null');
  assert(result.saturatedFat100g === null, 'Dawn saturatedFat100g must be null');
  assert(result.fiber === null, 'Dawn fiber must be null');
  assert(result.fiber100g === null, 'Dawn fiber100g must be null');
...
assert(g.fallbackExplanationForProductType('unsupported', { unsupportedReason: 'unverified_product' }) === g.UNVERIFIED_EXPLANATION);
assert(g.fallbackExplanationForProductType('unsupported', { unsupportedReason: 'known_non_food' }) === g.UNSUPPORTED_EXPLANATION);
...
assert(src.includes('fallbackExplanationForProductType(cached && cached.productType, cached)') ||
  src.includes('fallbackExplanationForProductType(cached && cached.productType)') ||
...
assert(g.SCAN_LOGIC_VERSION === '22', 'SCAN_LOGIC_VERSION must be 22 after serving-display consistency change');
assert(isCacheFresh({
  productType: 'food',
  cachedAt: now - 1000,
  scanLogicVersion: '10',
  explanation: "We've packed this product with 25.2g of sugar per serving.",
}, now) === false, 'v10 cache must rescan after bump so the explanation regenerates');
...
assert(!src.includes('[NUTRI FALLBACK]'), 'must not invent a Nutri-Score C fallback');
...
function loggedNutritionUnavailable(barcode) {
  return logs.some(l => l.includes('[NUTRITION SUBSCORE UNAVAILABLE]') && l.includes('barcode=' + barcode));
}
...
// 2. Product WITHOUT serving data → null nutrients, servingKnown false, prompt per 100g
{
  ...
  assert(result.calories === 'N/A', 'calories N/A without serving, got ' + result.calories);
  assert(result.calories100g === '400', 'calories100g from energy-kcal_100g via kJ/4.184, got ' + result.calories100g);
  assert(result.saturatedFat === 'N/A', 'saturatedFat N/A without serving');
  assert(result.saturatedFat100g === 'N/A', 'missing sat fat is N/A not derived');
  assert(result.fiber === 'N/A', 'fiber N/A without serving');
  assert(result.fiber100g === 'N/A', 'missing fiber is N/A');
  assert(result.caloriesTier === undefined, 'no caloriesTier');
  assert(result.saturatedFatTier === undefined, 'no saturatedFatTier');
  assert(result.fiberTier === undefined, 'no fiberTier');
...
// 3. serving_quantity 0 or "abc" → treated as missing
{
  ...
  assert(g.parseServingQuantity(500) === 500, '500 is the observed-data upper bound and is trusted');
  assert(g.parseServingQuantity(501) === null, 'quantities above 500 are not trusted');
  assert(g.parseServingQuantity(2000) === null, '2000ml whole-package serving is not trusted');
  ...
  const rejectedQty = g.resolveFoodServingNutrition({
    sugars_100g: 39, 'energy-kcal_100g': 140, sodium_100g: 0.045, proteins_100g: 0,
    sugars_serving: 780, 'energy-kcal_serving': 2800, sodium_serving: 0.9,
  }, 2000);
  assert(rejectedQty.servingKnown === false, 'untrusted quantity → serving unknown');
  assert(rejectedQty.sugarDisplay === null, 'untrusted quantity ignores explicit *_serving');
...
// 4. No Nutri-Score → nutriScoreKnown false; 60-point component is the USDA subscore
{
  clearLogs();
  ...
  assert(breakdown.nutritionAvailable === true, 'subscore still computed from nutrients');
  assert(typeof breakdown.nutriPts === 'number' && breakdown.nutriPts >= 0 && breakdown.nutriPts <= 60,
    'nutriPts is the 0-60 subscore, got ' + breakdown.nutriPts);
  assert(breakdown.nutriPts !== 40 || breakdown.nutritionPath === 'general', 'must not be an invented grade-C 40 from Nutri-Score');
  assert(!loggedNutritionUnavailable('nutri-missing'), 'complete nutrients must not log unavailable');
  assert(typeof result.score === 'number', 'score is still computed');
...
// 5. With Nutri-Score → nutriScoreKnown true; points still from USDA subscore, not grade B
{
  clearLogs();
  ...
  assert(breakdown.nutritionAvailable === true, 'subscore available');
  assert(typeof breakdown.nutriPts === 'number' && breakdown.nutriPts <= 60, 'nutriPts from subscore');
  assert(breakdown.nutriPts !== 50, 'must not map grade B to 50 Nutri-Score points');
  assert(!loggedNutritionUnavailable('nutri-known'), 'must not log unavailable when nutrients present');
}
...
// 6. /search path uses the same helper → same tiers and servingKnown
{
  ...
  assert(scanN.caloriesRaw === searchN.caloriesRaw, 'caloriesRaw parity');
  assert(scanN.saturatedFatRaw === searchN.saturatedFatRaw, 'saturatedFatRaw parity');
  assert(scanN.fiberRaw === searchN.fiberRaw, 'fiberRaw parity');
}

// 6b. Calories / saturated fat / fiber display — same serving pattern, no tiers
{
  assert(src.includes("getNumericNutrimentValue(derived, ['energy-kcal_100g', 'energy-kcal'])"),
    'subscore energy keys must stay energy-kcal_100g then energy-kcal');
  assert(src.includes('const energyKj = energyKcal * KJ_PER_KCAL'),
    'subscore must still convert energy to kJ via * 4.184');
  assert(src.includes('energyKj / KJ_PER_KCAL'),
    'display calories must convert kJ back with kcal = kJ / 4.184');
  assert(src.includes('function formatCalories'),
    'scan and search calories display must share formatCalories');
  assert(!src.includes(" / 10 + 'kcal'"),
    'formatCalories must not append a kcal suffix');
  assert(!src.includes('caloriesTier') && !src.includes('saturatedFatTier') && !src.includes('fiberTier'),
    'must not invent tiers for calories, saturated fat, or fiber');
  ...
  assert(n.caloriesRaw === (350 * 4.184) / 4.184, 'caloriesRaw is scorer kcal via kJ / 4.184');
  assert(n.caloriesDisplay === n.caloriesRaw * 50 / 100, 'calories per serving via toServing');
  assert(n.saturatedFatRaw === 1.5, 'sat fat from saturated-fat_100g');
  assert(n.saturatedFatDisplay === 0.75, 'sat fat 1.5/100g * 50g serving');
  assert(n.fiberRaw === 3.6, 'fiber from fiber_100g');
  assert(n.fiberDisplay === 1.8, 'fiber 3.6/100g * 50g serving');

  const fallbackKeys = g.resolveFoodServingNutrition({
    'energy-kcal': 80,
    'saturated-fat': 2,
    fiber: 4,
    proteins_100g: 1,
  ...
  assert(fallbackKeys.caloriesRaw === (80 * 4.184) / 4.184, 'energy-kcal fallback when _100g absent');
  assert(fallbackKeys.saturatedFatRaw === 2, 'saturated-fat fallback when _100g absent');
  assert(fallbackKeys.fiberRaw === 4, 'fiber fallback when fiber_100g absent');

  const kjOnly = g.resolveFoodServingNutrition({
    energy_100g: 1464.4,
    energy: 1464.4,
    'energy-kj_100g': 1464.4,
    proteins_100g: 1,
    sugars_100g: 1,
    sodium_100g: 0.1,
  }, 100);
  assert(kjOnly.caloriesRaw === null, 'must not substitute energy_100g / energy-kj for display');

  const missing = g.resolveFoodServingNutrition({
    proteins_100g: 1,
    sugars_100g: 1,
    sodium_100g: 0.1,
  }, 50);
  assert(missing.caloriesRaw === null && missing.saturatedFatRaw === null && missing.fiberRaw === null,
    'missing energy/sat-fat/fiber stay null');

  const scanned = await g.scanAndCacheFood('display-nutrients', {
  ...
  assert(scanned.calories === '175', 'scan calories per serving, got ' + scanned.calories);
  assert(scanned.calories100g === '350', 'scan calories100g 1-decimal rounding, got ' + scanned.calories100g);
  assert(scanned.saturatedFat === '0.8g', 'scan sat fat per serving rounded like grams, got ' + scanned.saturatedFat);
  assert(scanned.saturatedFat100g === '1.5g', 'scan sat fat 100g, got ' + scanned.saturatedFat100g);
  assert(scanned.fiber === '1.8g', 'scan fiber per serving, got ' + scanned.fiber);
  assert(scanned.fiber100g === '3.6g', 'scan fiber 100g, got ' + scanned.fiber100g);
  assert(scanned.caloriesTier === undefined && scanned.saturatedFatTier === undefined && scanned.fiberTier === undefined,
    'scan response has no invented tiers');
  const beforeScore = g.calculateScore('c', 4, 0, false, 10, 5, 0.1, [], 'display-nutrients', {
    'energy-kcal_100g': 350,
    proteins_100g: 10,
    sugars_100g: 5,
    sodium_100g: 0.1,
  ...
  assert(scanned.score === beforeScore, 'display fields must not change the score');
}
...
// Null nutrient → unknown tier (not low); omitted from explanation prompt
{
  ...
  assert(partial.servingKnown === false, 'explicit *_serving without a trusted quantity is unknown');
  assert(partial.sodiumDisplay === null, 'missing sodium display null');
  assert(partial.sugarDisplay === null, 'no trusted quantity → do not use sugars_serving');
  assert(partial.sodiumTier === 'unknown', 'missing sodium tier unknown, got ' + partial.sodiumTier);
  assert(partial.sugarTier === 'medium', '8g/100g sugar is medium when serving unknown, got ' + partial.sugarTier);
...
assert(photoBody.includes('appFacingUnsupportedReason'),
  '/scan/photo must set unsupportedReason from the classifier reason');
...

{
  ...
  assert(unsupported.scoreLabel === 'Food products only', 'unsupported scoreLabel');
  assert(unsupported.unsupportedReason === 'known_non_food', 'default reason is known_non_food');
...
{
  const unverified = g.buildUnsupportedScanResponse({
    productName: 'Dried Fruit & NUT Bread',
    extras: { unsupportedReason: 'unverified_product' },
  });
  assert(unverified.productType === 'unsupported', 'unverified stays unsupported type');
  assert(unverified.unsupportedReason === 'unverified_product');
  assert(unverified.scoreLabel === 'Unable to verify this product');
  assert(unverified.explanation === g.UNVERIFIED_EXPLANATION);
  assert(unverified.explanation === "Purla couldn't verify enough product data to determine whether this item can be scored.");
  assert(unverified.score === null);
}
...
// 6–9. Search classification via categories_tags
{
  ...
  function searchScoreFor(tags) {
    ...
    const score = g.calculateScore('b', 3, 0, false, 5, 5, 0.1, [], null, {
      'energy-kcal_100g': 80, proteins_100g: 5, sugars_100g: 5, sodium_100g: 0.1, fat_100g: 0, fiber_100g: 0,
    }, 'Yogurt');
    return {
      productType,
      score,
...
def test_scan_logic_v11_stale_explanation():
    """v11 bump regenerates explanations; version-mismatch stale fallback omits them."""
    script = r"""
const fs = require('fs');
const path = require('path');
const src = fs.readFileSync(path.join(process.cwd(), 'index.js'), 'utf8');
...
if (catchStart < 0 || catchEnd < 0) throw new Error('could not locate stale fallback catch');
const catchSlice = src.slice(catchStart, catchEnd);
assert(catchSlice.includes('logicMismatch'),
  'stale fallback catch must detect SCAN_LOGIC_VERSION mismatch');
assert(catchSlice.includes('!logicMismatch && !skipExplanation'),
  'ensureExplanation must not run on a version-mismatched fallback');

const fbStart = src.indexOf('function staleCacheFallbackPayload');
const fbEnd = src.indexOf('async function scanAndCache(barcode');
...
const cosStart = src.indexOf('const cosmeticTable = JSON.parse');
const cosEnd = src.indexOf('// Firestore docs are size-capped');
if ([nutStart, nutEnd, scoreStart, scoreEnd, fmtStart, fmtEnd, orgStart, orgEnd,
     extractStart, extractEnd, addDispStart, foodExplainStart, foodFnStart, foodFnEnd,
     cosStart, cosEnd].some((n) => n < 0)) {
  throw new Error('could not locate scanAndCacheFood extract bounds');
}
...
`;
fs.writeFileSync('/tmp/scan_logic_v11_helpers.js', block);
delete require.cache['/tmp/scan_logic_v11_helpers.js'];
const g = require('/tmp/scan_logic_v11_helpers.js');

const V10_WE_VOICE = "We've packed this product with 25.2g of sugar per serving.";

function isCacheFresh(cached, nowMs) {
  const age = nowMs - (cached.cachedAt || 0);
  const logicStale = cached.scanLogicVersion !== g.SCAN_LOGIC_VERSION;
  return age < g.CACHE_TTL_MS && !logicStale;
}
...
(async () => {
  ...
  const v10cached = {
    ...
    protein: '4g',
    sodium: '80mg',
    explanation: V10_WE_VOICE,
    scanLogicVersion: '10',
    cachedAt: now - 1000,
  };
  assert(isCacheFresh(v10cached, now) === false,
    'v10 cache must not be a hit after the bump');

  const regenerated = await g.scanAndCacheFood('7622210100586', {
    product_name: 'Cadbury Dairy Milk',
    ingredients_text: 'Sugar, cocoa butter, emulsifiers (E442, E476)',
    nutriscore_grade: 'e',
    nova_group: 4,
  ...
  assert(regenerated.scanLogicVersion === '22', 'rescan stamps v22');
  ...
  assert(!/we've packed/i.test(regenerated.explanation),
    'regenerated explanation must not keep v10 we-voice');

  const mismatchFb = g.staleCacheFallbackPayload({
    ...v10cached,
    cachedAt: 1,
  });
  assert(mismatchFb.productName === 'Cadbury Dairy Milk', 'fallback keeps product');
  assert(mismatchFb.score === 35, 'fallback keeps score');
  assert(mismatchFb.sugar === '25.2g' && mismatchFb.protein === '4g' && mismatchFb.sodium === '80mg',
    'fallback keeps nutrition');
  assert(mismatchFb.explanation === undefined, 'mismatch fallback omits explanation');
  assert(mismatchFb.explanationPending === true, 'mismatch fallback is explanation-pending');
  ...
  async function fallbackCatch(staleCached, skipExplanation) {
    const fallback = g.staleCacheFallbackPayload(staleCached);
    const logicMismatch = staleCached.scanLogicVersion !== g.SCAN_LOGIC_VERSION;
    if (!logicMismatch && !skipExplanation && !g.hasUsableExplanation(fallback)) {
      fallback.explanation = await ensureExplanation();
      fallback.explanationPending = false;
    }
    return fallback;
  }
  const afterCatch = await fallbackCatch({ ...v10cached, cachedAt: 1 }, false);
  assert(ensureCalls === 0, 'fallback catch must not call ensureExplanation on mismatch');
  assert(afterCatch.explanation === undefined, 'catch must not reattach v10 text');
  ...
  assert(isCacheFresh(v22fresh, now) === true, 'fresh v22 record is a cache hit');
  assert(isCacheFresh(v21fresh, now) === false, 'fresh v21 record is stale after the v22 bump');
  assert(isCacheFresh(v20fresh, now) === false, 'fresh v20 record is stale after the v22 bump');
  assert(isCacheFresh(v19fresh, now) === false, 'fresh v19 record is stale after the v22 bump');
  assert(isCacheFresh(v18fresh, now) === false, 'fresh v18 record is stale after the v22 bump');
  assert(isCacheFresh(v17fresh, now) === false, 'fresh v17 record is stale after the v22 bump');
  assert(isCacheFresh(v16fresh, now) === false, 'fresh v16 record is stale after the v22 bump');
  assert(isCacheFresh(v15fresh, now) === false, 'fresh v15 record is stale after the v22 bump');
  assert(isCacheFresh(v14fresh, now) === false, 'fresh v14 record is stale after the v22 bump');
  const v22Fb = g.staleCacheFallbackPayload({ ...v22fresh, cachedAt: 1 });
  assert(v22Fb.explanation === REGENERATED, 'same-version stale fallback serves its explanation');
  assert(v22Fb.explanationPending !== true, 'same-version fallback must not mark explanation pending');
...
"""
    ...
    if proc.returncode != 0:
        ...
        sys.stderr.write(proc.stderr)
        raise AssertionError(
            f"scan logic v11 stale explanation assertions failed (exit {proc.returncode})"
        )
    print(proc.stdout.strip())
...
assert(/const SCAN_LOGIC_VERSION = '22'/.test(src), 'SCAN_LOGIC_VERSION must be 22 so cached v21 scans rebuild serving display');
...
def test_usda_food_lookup():
    ...
    proc = subprocess.run(
        ["node", str(ROOT / "test_usda_lookup_assert.js")],
        cwd=str(ROOT),
        capture_output=True,
        text=True,
    )
...
def test_nutrition_subscore_validation():
    """Spearman and pairwise ordering vs OFF Nutri-Score on the Task 1 USDA sample."""
    proc = subprocess.run(
        ["node", str(ROOT / "validation" / "validate_nutrition_subscore.js")],
        cwd=str(ROOT),
        capture_output=True,
        text=True,
    )
    ...
    if proc.returncode != 0:
        sys.stderr.write(proc.stderr)
        raise AssertionError(
            f"nutrition subscore validation failed (exit {proc.returncode})"
        )
