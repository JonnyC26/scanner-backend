...
function unsupportedScanFromRecord(record, extras = {}) {
  ...
  return buildUnsupportedScanResponse({
    ...
    ingredients: typeof record.ingredients_text === 'string' ? record.ingredients_text : '',
    extras: {
...
// Search-universe classification lives in lib/search_product_type.js so the
// image-mirror job can reuse classifySearchProductType without loading this
// file. Bindings keep the same names; behaviour is unchanged.
const {
...
function formatCalories(val) {
  if (val === null || val === undefined) return 'N/A';
  return Math.round(val * 10) / 10 + ' Cal';
}
